...
export function stringifyObject(input: any, options: StringifyObjectOptions): string;
export function stringifyObject(input: any, options: StringifyObjectOptions, pad: string): string;
...
export function stringifyObject(input: any, options?: StringifyObjectOptions, pad: string = ''): string {
    ...
    const defaultOptions: StringifyObjectOptions = {
        indent: '\t',
        singleQuotes: true
    };
    ...
    if (Array.isArray(input)) {
        ...
        const ret = '[' + tokens.newLine + input.map((el, i) => {
            ...
            let value = stringifyObject(el, combinedOptions, pad + combinedOptions.indent);
    ...
    if (isObj(input)) {
        ...
        const ret = '{' + tokens.newLine + objKeys.map((el, i) => {
            ...
            const key = isSymbol || isClassic ? el : stringifyObject(el, combinedOptions);

            let value = stringifyObject(input[el], combinedOptions, pad + combinedOptions.indent);
            if (combinedOptions.transform) {
